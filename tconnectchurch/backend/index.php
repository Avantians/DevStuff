...
if ( !isset($_SESSION['eWaySession']) && $_SESSION['eWaySession'] !== "eWayPlus". strtotime(date("mdy")) ){
	...
	if( $_ci_path === UNDER_SUBFOLDER . "/backend/index.php" || $_ci_path === UNDER_SUBFOLDER . "/backend/" ){
 ...
 *  ----------------------------------------------------------------------- */
if ( !file_exists( CONFIG_DCOCUMENT_ROOT ."/backend/design/". $Config_template_name ."/login.php" ) ){
	http_response_code(404);		// Set a 400 (bad request) response code and exit.
	echo CONFIG_TEMPLATE_WARN_TXT . $Config_template_name;
	die();
}
else {
	require_once( CONFIG_DCOCUMENT_ROOT ."/backend/design/". $Config_template_name ."/login.php" );
